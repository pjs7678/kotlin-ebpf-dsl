...
class CCodeGenerator(private val model: BpfProgramModel) {
    ...
    companion object {
        private val PROBE_READ_HELPERS = setOf(
    ...
    fun generate(): String {
        ...
        sb.appendLine("// AUTO-GENERATED by kotlin-ebpf-dsl — do not edit")
        sb.appendLine()

        // Includes — conditional on target kernel
        if (model.targetKernel >= KernelVersion.V5_2) {
            ...
            // Pre-BTF: use standard kernel headers
            sb.appendLine("#include <linux/bpf.h>")
            sb.appendLine("#include <bpf/bpf_helpers.h>")
            sb.appendLine("#include <bpf/bpf_tracing.h>")
        }
        sb.appendLine()
        ...
        when (stmt) {
            is BpfStmt.VarDecl -> {
                ...
                when (val type = v.type) {
                    ...
                    is BpfScalar -> {
                        val init = stmt.init
                        if (init is BpfExpr.HelperCall && init.helperName in PROBE_READ_HELPERS) {
                            // bpf_probe_read_kernel/user takes (dst, size, src)
