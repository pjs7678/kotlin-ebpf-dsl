...
int kprobe_cap_capable(struct pt_regs *ctx)
{
    ...
    struct cap_stats *entry_1 = bpf_map_lookup_elem(&cap_stats, &var_0);
    if (entry_1) {
        __sync_fetch_and_add(&entry_1->checks, 1ULL);
    } else {
        struct cap_stats var_2 = {};
        var_2.checks = 1ULL;
        bpf_map_update_elem(&cap_stats, &var_0, &var_2, 1);
    }
    return 0;
}
