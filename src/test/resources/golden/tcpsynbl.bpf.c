...
int kprobe_tcp_v4_syn_recv_sock(struct pt_regs *ctx)
{
    __u64 cgroup_id = bpf_get_current_cgroup_id();
    struct cgroup_key var_0 = {};
    var_0.cgroup_id = cgroup_id;
    struct counter *entry_1 = bpf_map_lookup_elem(&syn_count, &var_0);
    if (entry_1) {
        __sync_fetch_and_add(&entry_1->count, 1ULL);
    } else {
        struct counter var_2 = {};
        var_2.count = 1ULL;
        bpf_map_update_elem(&syn_count, &var_0, &var_2, 1);
    }
    return 0;
}
